import { supabase } from '@/integrations/supabase/client';

const LPO_MIGRATION_SQL = `
-- Create LPO status enum
DO $$ BEGIN
    CREATE TYPE lpo_status AS ENUM ('draft', 'sent', 'approved', 'received', 'cancelled');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

-- Main LPO table
CREATE TABLE IF NOT EXISTS lpos (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    company_id UUID REFERENCES companies(id) ON DELETE CASCADE,
    supplier_id UUID REFERENCES customers(id) ON DELETE CASCADE,
    lpo_number VARCHAR(100) UNIQUE NOT NULL,
    lpo_date DATE NOT NULL DEFAULT CURRENT_DATE,
    delivery_date DATE,
    status lpo_status DEFAULT 'draft',
    subtotal DECIMAL(15,2) DEFAULT 0,
    tax_amount DECIMAL(15,2) DEFAULT 0,
    total_amount DECIMAL(15,2) DEFAULT 0,
    notes TEXT,
    terms_and_conditions TEXT,
    delivery_address TEXT,
    contact_person VARCHAR(255),
    contact_phone VARCHAR(50),
    created_by UUID REFERENCES users(id),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- LPO items table
CREATE TABLE IF NOT EXISTS lpo_items (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    lpo_id UUID REFERENCES lpos(id) ON DELETE CASCADE,
    product_id UUID REFERENCES products(id),
    description TEXT NOT NULL,
    quantity DECIMAL(10,3) NOT NULL,
    unit_price DECIMAL(15,2) NOT NULL,
    tax_rate DECIMAL(5,2) DEFAULT 0,
    tax_amount DECIMAL(15,2) DEFAULT 0,
    line_total DECIMAL(15,2) NOT NULL,
    notes TEXT,
    sort_order INTEGER DEFAULT 0
);

-- Indexes for performance
CREATE INDEX IF NOT EXISTS idx_lpos_company_id ON lpos(company_id);
CREATE INDEX IF NOT EXISTS idx_lpos_supplier_id ON lpos(supplier_id);
CREATE INDEX IF NOT EXISTS idx_lpos_lpo_number ON lpos(lpo_number);
CREATE INDEX IF NOT EXISTS idx_lpos_status ON lpos(status);
CREATE INDEX IF NOT EXISTS idx_lpos_lpo_date ON lpos(lpo_date);
CREATE INDEX IF NOT EXISTS idx_lpo_items_lpo_id ON lpo_items(lpo_id);
CREATE INDEX IF NOT EXISTS idx_lpo_items_product_id ON lpo_items(product_id);

-- RPC function to generate LPO number
CREATE OR REPLACE FUNCTION generate_lpo_number(company_uuid UUID)
RETURNS TEXT AS $$
DECLARE
    company_code TEXT;
    lpo_count INTEGER;
    lpo_number TEXT;
BEGIN
    -- Get company code/prefix
    SELECT COALESCE(UPPER(LEFT(name, 3)), 'LPO') INTO company_code
    FROM companies 
    WHERE id = company_uuid;
    
    -- Count existing LPOs for this company
    SELECT COUNT(*) INTO lpo_count
    FROM lpos
    WHERE company_id = company_uuid;
    
    -- Generate LPO number
    lpo_number := company_code || '-LPO-' || TO_CHAR(CURRENT_DATE, 'YYYY') || '-' || LPAD((lpo_count + 1)::TEXT, 4, '0');
    
    RETURN lpo_number;
END;
$$ LANGUAGE plpgsql;

-- Update trigger for updated_at
CREATE OR REPLACE FUNCTION update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Add trigger to lpos table
DROP TRIGGER IF EXISTS update_lpos_updated_at ON lpos;
CREATE TRIGGER update_lpos_updated_at
    BEFORE UPDATE ON lpos
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- Add some comments for documentation
COMMENT ON TABLE lpos IS 'Local Purchase Orders for ordering goods from suppliers';
COMMENT ON TABLE lpo_items IS 'Line items for Local Purchase Orders';
COMMENT ON COLUMN lpos.supplier_id IS 'References customers table - suppliers are stored as customers with supplier flag';
COMMENT ON COLUMN lpos.status IS 'LPO status: draft, sent, approved, received, cancelled';
COMMENT ON FUNCTION generate_lpo_number(UUID) IS 'Generates unique LPO number for a company';
`;

export const runLPOMigration = async () => {
  console.log('🚀 Starting LPO tables migration...');

  try {
    // Try different RPC function names that might exist
    const rpcMethods = ['exec_sql', 'sql', 'execute_sql', 'run_sql'];
    const argNames = ['sql_query', 'query'];

    let migrationSuccess = false;
    let lastError = null;
    let errorDetails = '';

    for (const rpcMethod of rpcMethods) {
      for (const argName of argNames) {
        try {
          console.log(`Trying ${rpcMethod} with argument ${argName}...`);

          const { data, error } = await supabase.rpc(rpcMethod, {
            [argName]: LPO_MIGRATION_SQL
          });

          if (!error) {
            console.log('✅ LPO migration completed successfully!');
            console.log('Migration result:', data);
            migrationSuccess = true;
            break;
          } else {
            lastError = error;
            errorDetails = `${rpcMethod}(${argName}): ${JSON.stringify(error, null, 2)}`;
            console.log(`❌ ${rpcMethod}(${argName}) failed:`, error);
            console.log('Error details:', JSON.stringify(error, null, 2));
          }
        } catch (err) {
          lastError = err;
          const errString = err instanceof Error ? err.message : String(err);
          errorDetails = `${rpcMethod}(${argName}): ${errString}`;
          console.log(`❌ ${rpcMethod}(${argName}) threw error:`, err);
        }
      }

      if (migrationSuccess) break;
    }

    if (!migrationSuccess) {
      const errorMessage = lastError instanceof Error ? lastError.message :
                          (lastError && typeof lastError === 'object') ? JSON.stringify(lastError, null, 2) :
                          String(lastError);
      throw new Error(`All RPC methods failed. Last error: ${errorMessage}. Details: ${errorDetails}`);
    }
    
    // Verify tables were created
    console.log('🔍 Verifying LPO tables...');
    const { data: lposExists, error: lposError } = await supabase
      .from('lpos')
      .select('id')
      .limit(1);
      
    const { data: itemsExists, error: itemsError } = await supabase
      .from('lpo_items')
      .select('id')
      .limit(1);
    
    if (!lposError && !itemsError) {
      console.log('✅ LPO tables verification successful!');
      return { success: true, message: 'LPO tables created and verified successfully!' };
    } else {
      console.log('⚠️ Tables may have been created but verification failed:', { lposError, itemsError });
      return { success: true, message: 'LPO tables migration completed (verification unclear)' };
    }
    
  } catch (error) {
    console.error('❌ LPO migration failed:', error);

    let errorMessage = 'Unknown error';
    if (error instanceof Error) {
      errorMessage = error.message;
    } else if (typeof error === 'object' && error !== null) {
      errorMessage = JSON.stringify(error, null, 2);
    } else {
      errorMessage = String(error);
    }

    return {
      success: false,
      error,
      message: `Migration failed: ${errorMessage}`
    };
  }
};
