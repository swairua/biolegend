...
export default function DeliveryNotes() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'failed':
        return <Badge variant="outline" className="bg-destructive-light text-destructive border-destructive/20"><AlertTriangle className="h-3 w-3 mr-1" />Failed</Badge>;
      default:
  ...
  const delivered = deliveryNotes?.filter(note => note.status === 'delivered').length || 0;
  const prepared = deliveryNotes?.filter(note => note.status === 'prepared').length || 0;
